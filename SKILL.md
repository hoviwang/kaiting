...
用户选择「够了，进检讨」后，主agent**必须输出以下格式并真的动手执行**，不得只说不做。
...
检讨输出完成后，主agent**必须真的动手执行第3步列出的行动**，而不是结束。

- 如果检讨说要「查某个信息源」→ 立即调用 web_search / minimax__web_search 实际去查
- 如果检讨说要「修正某个错误」→ 立即执行修正
- 如果检讨说要「重试某个操作」→ 立即重试

执行完后报告结果，再问用户：「检讨执行完毕，要继续深挖吗？」
